...
Normalized server session shape used by session helpers and `appSession.enrich`.
...
### `AppSessionEnrich` / `AppSessionEnrichResult`

Types used by `defineServerAuth({ appSession: { enrich } })`:

- `AppSessionEnrich`: enrichment callback signature.
- `AppSessionEnrichResult`: partial user/session updates returned by the callback.
