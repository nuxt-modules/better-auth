...
`useUserSession()` keeps `user`, `session`, and readiness state in Nuxt state. The Better Auth client and auth methods are runtime services, not serializable state.

When you forward auth values from a Pinia setup store, return the state refs directly and avoid spreading the full composable result into the store. If your store exposes the client, wrap it as non-hydrated state with Pinia's `skipHydrate()` and Vue's `shallowRef()` / `markRaw()`.
...
export const useUserStore = defineStore('user', () => {
  ...
  const authClient = shallowRef(auth.client ? markRaw(auth.client) : null)
  ...
  return {
    ...
    authClient: skipHydrate(authClient),
  }
})
```
