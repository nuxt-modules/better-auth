...
| Use case | Composable |
|----------|------------|
| Pages and components that need auth state, sign-in, sign-up, sign-out, or direct Better Auth client access | `useUserSession()` |
| Pinia setup stores or shared state containers that only need session state and session actions | `useUserSessionState()` |
...
Use `useSignIn()`, `useSignUp()`, or `useAuthClientAction()` in form components for auth actions that need loading and error state.
