...
Use `useUserSession()` in pages and components. Use `useUserSessionState()` when you forward auth state from a Pinia setup store.
...
| Pinia setup stores and shared state containers | `useUserSessionState()` |
...
Use `useUserSessionState()` when you want auth state in a Pinia store or another shared state container. It exposes the session state and session actions from `useUserSession()` without exposing the Better Auth client namespaces.

### When to use `useUserSession()` vs `useUserSessionState()`

| Use `useUserSession()` | Use `useUserSessionState()` |
|------------------------|-----------------------------|
| Pages and components | Pinia setup stores |
| Login, signup, and logout UI | Shared auth state containers |
| Direct Better Auth client/plugin access | Route guards or UI state derived from the current session |
| `client`, `signIn`, or `signUp` access | `user`, `session`, `loggedIn`, and `ready` access |
