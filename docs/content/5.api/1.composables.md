...
| Sign-out buttons that need loading, error, and success state | `useSignOut()` |
...
Wraps `useUserSession().signOut()` with the same action handle shape as `useSignIn()` and `useSignUp()`.

```vue [components/SignOutButton.vue]
<script setup lang="ts">
const { execute: signOut, status, error } = useSignOut()
</script>

<template>
  <button :disabled="status === 'pending'" @click="signOut()">
...
On success, it clears the local session and follows `auth.redirects.logout` when configured. Pass `execute({ onSuccess })` to run your own callback instead of the configured redirect. The action stays pending until the callback completes.

On a failed sign-out request, `status` becomes `'error'` and `error` contains the normalized error. The session remains intact, and neither the success callback nor redirect runs. As with the other action composables, `execute()` resolves after capturing errors; read `status` or `error` to determine the outcome. Concurrent calls share the existing sign-out request.

Create the composable during setup; call `execute()` only on the client. Signing out an anonymous user does not delete their account. Use the anonymous plugin's `deleteAnonymousUser` method explicitly when that is your application's policy.
