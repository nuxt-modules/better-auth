...
Use `useUserSessionState()` when you want auth state in a store or another shared state container. It returns only `user`, `session`, `loggedIn`, `ready`, `fetchSession`, `waitForSession`, `signOut`, and `updateUser`.

It does not return `client`, `signIn`, or `signUp`. Those values are runtime Better Auth client namespaces, not serializable auth state. Use `useUserSession()` in components when you need the full client, or use `useSignIn()`, `useSignUp()`, and `useAuthClientAction()` for form and action flows.
...
`useUserSessionState()` is the recommended API for Pinia setup stores because it exposes only store-safe auth state and actions.
