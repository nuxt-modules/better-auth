...
The action handle reports failures in two ways: thrown errors and `{ error }` results.

When the underlying method throws, `execute()` rejects. Use `try/catch` when control flow depends on success:
...
When the underlying method resolves to an object with a truthy `error` field, `execute()` does not throw. Check the return value if the method follows the Better Auth `{ error }` pattern:
