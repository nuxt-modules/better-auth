...
```ts [pages/login.vue]
const { execute, executeSafe, pending, status, error, errorMessage } = useUserSignIn().email
...
::field-group
  ...
  ::
  ::field{name="executeSafe" type="(...args) => Promise<{ ok: true, data: any } | { ok: false, error: AuthActionError }>"} 
    Calls the underlying method but never throws.
  ::
...
Use `status`, `error`, and `errorMessage` as your source of truth. The action handle always sets `status='error'` and populates normalized `error` when a sign-in attempt fails.
...
#### Non-throwing flow (`executeSafe`) (recommended)

If you don't want try/catch in page code, use `executeSafe()`:

```ts [pages/login.vue]
const { executeSafe } = useUserSignIn().email

const result = await executeSafe({ email, password })
if (!result.ok) {
...
```ts [pages/signup.vue]
const { execute, executeSafe, pending, status, error, errorMessage } = useUserSignUp().email
