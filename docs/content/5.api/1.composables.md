...
Better Auth methods can signal failure in two ways:

- They can throw (the returned promise rejects).
- They can resolve to a result that contains a truthy `error` field.

You do not need to care which one happened to render UI. You can always read `status` and `error`.
