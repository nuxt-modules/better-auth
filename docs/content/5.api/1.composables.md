...
::warning
`useUserSignIn` and `useUserSignUp` were renamed to `useSignIn` and `useSignUp` before `0.0.2`.
The API switched from map-style access (`useUserSignIn().email`) to keyed access (`useSignIn('email')`) before `0.0.2`.
OAuth provider aliases (for example `useSignIn('github')`) were removed. Use `useSignIn('social').execute({ provider: 'github' })`.
`error` changed from `unknown | null` to `AuthActionError | null` before `0.0.2`.
The message alias field was removed before `0.0.2`. Use `error.value?.message`.
`execute()` changed during prereleases:
- old: `await execute()` could reject
- previous prerelease: `await execute()` resolved `{ ok: true, data } | { ok: false, error }`
