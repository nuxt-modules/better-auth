...
```ts [pages/login.vue]
const { execute, pending, status, error, errorMessage } = useUserSignIn().email
...
::field-group
  ::field{name="execute" type="(...args) => Promise<{ ok: true, data: any } | { ok: false, error: AuthActionError }>"}
...
Use `status`, `error`, and `errorMessage` as your source of truth. The action handle always sets `status='error'` and populates normalized `error` when a sign-in attempt fails, while `execute()` returns `{ ok: false, error }`.

::note
Better Auth methods can signal failure by throwing or by resolving to a `{ error }` result. In both cases, the action handle updates `status` and `error`, and `await execute()` resolves to `{ ok: false, error }`.
::

#### Recommended flow (`execute`)

```ts [pages/login.vue]
const { execute, status, errorMessage } = useUserSignIn().email
...
`execute()` also changed to a non-throwing result union:
- old: `await execute()` could reject
- new: `await execute()` always resolves `{ ok: true, data } | { ok: false, error }`
...
```ts [pages/signup.vue]
const { execute, pending, status, error, errorMessage } = useUserSignUp().email
