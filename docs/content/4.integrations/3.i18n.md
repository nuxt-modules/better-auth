...
## Using `resolveAuthErrorMessage`

For a reusable message resolver across auth and plugin actions, use `resolveAuthErrorMessage` with an optional translation callback:
...
This keeps UI fallback text (`Please try again.`) consistent while still allowing code-based localization.
