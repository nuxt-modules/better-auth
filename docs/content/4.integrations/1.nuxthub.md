...
::warning
Both `hub.kv: true` and `auth.secondaryStorage: true` must be set together. The build will fail with a clear error if `secondaryStorage` is enabled without `hub.kv`.
::
