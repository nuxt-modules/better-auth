...
If you want to use a different KV backend (Redis, Upstash, etc.), set `hubSecondaryStorage: 'custom'` and provide your own `secondaryStorage` in `defineServerAuth()`. The module will omit the session table from the generated schema but won't inject NuxtHub KV code.
