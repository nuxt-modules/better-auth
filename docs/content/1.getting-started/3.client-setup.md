...
```txt [Prompt]
Set up the client auth config for @nuxtjs/better-auth.

- Create `app/auth.config.ts` with a default export using `defineClientAuth` from `@nuxtjs/better-auth/config`
