...
::field-group
  ...
  ::field{name="hubSecondaryStorage" type="boolean | 'custom'"}
    ...
    - `'custom'` — You provide your own `secondaryStorage` in `defineServerAuth()`. The module won't inject NuxtHub KV but will omit the session table from the generated schema.
