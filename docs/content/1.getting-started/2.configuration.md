...
Use `appSession.enrich` to project or enrich session data returned by session APIs without re-implementing request-level session lookups in each handler.
...
export default defineServerAuth({
  appSession: {
    async enrich({ user, session }, _ctx) {
      return {
        user: {
          ...user,
...
::note
`appSession.enrich` is optional. If omitted, session behavior remains unchanged.
::
