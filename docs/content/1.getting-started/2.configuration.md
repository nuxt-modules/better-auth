...
::field-group
  ...
  ::field{name="redirects" type="{ login?: string, guest?: string, authenticated?: string, logout?: string }"}
    ...
    - `authenticated`: where to navigate after successful `signIn` / `signUp` when no `onSuccess` callback is provided
...
- `auth.redirects.authenticated` is used when `signIn` / `signUp` complete without an explicit `onSuccess`.
