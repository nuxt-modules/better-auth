...
::field-group
  ...
  ::field{name="redirects" type="{ login?: string, guest?: string, authenticated?: string, logout?: string, sessionExpired?: string }"}
    ...
    - `sessionExpired`: where to navigate when a previously authenticated session expires or is revoked (no default)
