...
## Redirecting after passive session loss

The client refreshes sessions when a tab regains focus and when another tab changes the session. If a previously authenticated session is then found to be expired or revoked, configure an optional redirect:
...
This redirect only runs for a settled transition from an active session to a signed-out state. It does not run for an initially signed-out visitor, and explicit `signOut()` continues to use `auth.redirects.logout`.
