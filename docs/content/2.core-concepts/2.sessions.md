...
Use a split model:
- in pages/components, use `useUserSession()` state and SSR-safe fetch helpers (`useAuthRequestFetch`, `useAuthAsyncData`)
- in server handlers/routes, use server utilities (`serverAuth`, `getUserSession`, `requireUserSession`)
