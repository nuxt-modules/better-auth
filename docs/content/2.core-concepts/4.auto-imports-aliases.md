...
- Client (auto-imported in Vue): `useUserSession()`, `useUserSessionState()`, `useAuthClient()`, `useSignIn()`, `useSignUp()`, `useSignOut()`, `useAction()`, `useAuthClientAction()`, `useAuthRequestFetch()`, `useAuthAsyncData()`, `runWithSessionRefresh()`
...
For app code, prefer the auto-imports. For published reusable packages or Nuxt modules, import composables from `@nuxtjs/better-auth/composables` instead of `#imports`.
