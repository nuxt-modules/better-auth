...
Do not combine an auth rule with `cache`, `swr`, `isr`, `static`, `prerender`, or `proxy`. Those rules can run before authentication or reuse a response across users. The module rejects direct and inherited combinations during setup. In development, it skips paths owned by dev server handlers, such as `/_fonts` from `@nuxt/fonts`, because those requests never reach Nitro route rules.
