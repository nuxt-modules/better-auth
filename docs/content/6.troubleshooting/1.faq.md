...
## "Invalid origin" locally (followed by provider "invalid redirect_uri")

This error usually means Better Auth resolves a different local host than your browser origin. The most common mismatch is `127.0.0.1` vs `localhost`.

Follow this workflow to debug the mismatch:

::steps
### Set an explicit local site URL
Add an explicit site URL in `nuxt.config.ts`:
...
### Restart and inspect auth config
Restart dev and inspect [http://localhost:3000/api/_better-auth/config](http://localhost:3000/api/_better-auth/config):
...
::important
OAuth providers must use exact callback URL matching. Ensure provider settings match your app callback exactly (including protocol, host, port, and path), for example:
